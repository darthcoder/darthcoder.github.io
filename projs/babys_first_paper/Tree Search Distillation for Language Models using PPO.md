...
published:
created: 2026-03-15
description: "Personal website of Ayush Tambde"
tags:
  - "clippings"
---
03-01-2026 · Updated 03-03-2026

Game-playing neural networks like AlphaZero achieve superhuman performance in board games by augmenting the raw policy with a test-time search harness and distilling the stronger, augmented policy back into the network. Why aren’t similar techniques used in language modelling today? The DeepSeek-R1 authors mention they found [limited success with MCTS](https://arc.net/l/quote/caevdneg); Finbarr Timbers has an [excellent post](https://finbarr.ca/request-for-research-puct/) on why they may have faced this problem, namely their choice of UCT instead of pUCT.
...
To explore this, I applied MCTS across reasoning steps to Qwen-2.5-1.5B-Instruct, to search for stronger trajectories and distill these back into the model via an online PPO loop. On the task of [Countdown](https://en.wikipedia.org/wiki/Countdown_\(game_show\)), a combinatorial arithmetic game, the distilled model (evaluated without a search harness) achieves an asymptotic mean@16 eval score of 11.3%, compared to 8.4% for CISPO and 7.7% for best-of-N. Relative to the pre-RL instruct model (3.1%), this is an 8.2 percentage point improvement.
...
I prefer the approach introduced by [Tree-of-Thoughts (Yao et al., 2023)](https://arxiv.org/abs/2305.10601) to search over possible next reasoning steps. In this formulation each node-state is a sequence of contiguous tokens:

- The root node corresponds to the input prompt
- Intermediate nodes correspond to reasoning steps: `<step>...</step>`
- Terminal nodes correspond to answers: `<answer>...</answer>`
...
This approach shares similarities with [TS-LLM (Feng et al., 2023)](https://arxiv.org/html/2309.17179v2), which also combines AlphaZero-style tree search with a learned value function over sentence-level actions.

The key differences are:

1. Using online RL (CISPO/PPO) rather than SFT for distillation
2. Parallel MCTS with virtual losses as an additional scaling axis
...
![Countdown mean@16 evaluation curves for MCTS-distilled policy, CISPO, and best-of-N](https://ayushtambde.com/blog-countdown-results.png)
...
All code is open source and can be found [here](https://github.com/at2005/llm-mcts).

## Future Directions

So what does this mean? The part that excites me here is the additional reasoning knobs we can tune, like the number of parallel workers per tree, or the number of MCTS iterations. I haven't tuned these properly, but initial experiments showed increasing both these values led to significant performance gains. So I want to explore this direction further! There's plenty of work to be done scaling this method and charting empirical trends to evaluate its potential for larger models and compute budgets. [Reach out](mailto:tambdeayush@gmail.com) if you would like to collaborate!
...
| CISPO epsilon\_high | 5.0 | Clipping parameter for CISPO |
...
| c <sub>puct</sub> | 0.5 | pUCT coefficient |
| c <sub>KL</sub> | 0.05 | KL divergence weight |
| c <sub>ppo</sub> | 1.0 | PPO objective weight |
| c <sub>value</sub> | 1.0 | Value objective weight |
| N <sub>t</sub> | 2 | Number of trainer processes |
| N <sub>g</sub> | 6 | Number of generator processes |
...
![MCTS mean@16 evaluation by step](https://ayushtambde.com/plots-mcts/mcts-eval.png)

MCTS mean@16 evaluation curve.

![GRPO mean@16 evaluation by step](https://ayushtambde.com/plots-mcts/grpo-eval.png)

GRPO/CISPO mean@16 evaluation curve.

![Best-of-N mean@16 evaluation by step](https://ayushtambde.com/plots-mcts/bon-eval.png)

Best-of-N mean@16 evaluation curve.

### Training

![MCTS reward by iteration](https://ayushtambde.com/plots-mcts/mcts-reward-train.png)

MCTS reward-training curve.

![GRPO reward by iteration](https://ayushtambde.com/plots-mcts/grpo-train-reward.png)

GRPO/CISPO reward-training curve.

![Best-of-N reward by iteration](https://ayushtambde.com/plots-mcts/bon-reward-train.png)

Best-of-N reward-training curve.