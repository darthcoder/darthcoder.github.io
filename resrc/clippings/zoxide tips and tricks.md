...
source: "https://batsov.com/articles/2025/06/12/zoxide-tips-and-tricks/"
author:
  - "[[Bozhidar Batsov]]"
published: 2025-06-12
created: 2026-03-14
description: "zoxide is a smart and fast alternative to cd that learns your directory usage patterns and allows you to jump to directories quickly. It’s one of my favorite command-line tools and it’s an essential part of my workflow."
tags:
  - "clippings"
  - "zoxide"
---
...
- Normally you’d use `z` to trigger `zoxide`, but pressing `z` is not super convenient. That’s why I like to use `j` instead. You can set this up by adding the following to your shell configuration file (e.g., `.bashrc`, `.zshrc`):
	```sh
	1
	alias j='z'
	```
...
- If you have `fzf` installed, the `zi` command can be used to interactively select a directory to jump to. `zi` is another thing that’s not super easy to type, so I suggest using the `jj` alias instead:
	```sh
	1
	alias jj='zi'
	```
